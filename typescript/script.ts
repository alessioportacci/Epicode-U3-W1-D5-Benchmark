interface ISmartphone
{
    ...
    //numeroChiamate:number   Ora che ho un registro chiamate, lui diventa inutile, lo ottengo con .length
    registroChiamate: {id:string, durata:number, data: Date}[]

    //Getter
    getNumeroChiamate()   :number
    getNumeroTelefono()   :String
    getCreditoResiduo()   :number                                    //Corrispettivo di numero404()
    getCostoMinuto()      :number
    getRegistroChiamate() :{id:string, durata:number, data: Date}[]  //Corrispettivo di mostraRegistroChiamate()
    
    //Setter
    setCostoMinuto(nuovaTariffa:number) :void
    //setNumeroChiamate(nChiamate:number) :void   Volevo impementare questa cosa, ma crea conflitti logici con l'extra

    //Altri
    ricarica(euro:number) :void
    chiamata(min:number)  :void
    azzeraChiamate()      :void
    filtraChiamatePerDataOra(data:Date) :{id:string, durata:number, data: Date}[]
}

class Smartphone implements ISmartphone
{
    public registroChiamate: { id: string, durata: number, data: Date }[]

    constructor
    (
        public creditoResiduo :number,    
        public numeroTelefono :string,    
        public costoMinuto    :number = 0.20 //Di default sono 20 cent a chiamata (un po' caro?)
    )
    {
    ...
    getNumeroChiamate(): number 
    //Getter method to get numeroChiamate value
    {
        return this.registroChiamate.length
    }

    getCreditoResiduo(): number 
    //Getter method to get creditoResituo value 
    {
        return this.creditoResiduo
    }

    getNumeroTelefono(): String 
    //Getter method to get numeroTelefono value
    {
        return this.numeroTelefono
    }

    getCostoMinuto() :number
    //Getter method to get costoMinuto value
    {
        return this.costoMinuto
    }

    getRegistroChiamate(): { id: string; durata: number; data: Date }[] 
    {
    ...
    setCostoMinuto(nuovaTariffa:number): void 
    //Setter method to set a new costoMinuto value
    {
    ...
    //#region Altri metodi
    ricarica(euro: number): void 
    //Method to add more phone credit
    {
        this.creditoResiduo += euro
    }

    chiamata(min: number): void 
    //Method to calculate the call cost
    {
